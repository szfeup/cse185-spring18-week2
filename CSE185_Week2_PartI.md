...
## 1. Setting up
First of all, since you only have a limited (~10GB) disk space quota for the course, let's clean out our directories from last week. You won't be needing any of the data from Lab 1 (and even if you do, your lab notebook should contain everything you need to reproduce your results!). So go ahead and use `rm` to remove data files. You can remove files one-by-one (`rm filename`), but that will take a while. Typing `rm week1/*` should remove all the files in your week 1 folder. Please be careful with this command! You should always triple check before running an `rm` command to make sure you don't irreversibly delete something you will need later.
